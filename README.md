...
sudo nix run nixpkgs#restic --extra-experimental-features "nix-command flakes" -- restore latest -r "$REPO" --target "$RESTORE_PATH" --include "/home/nixos/.config/nixos" --include "/var/lib/sops-nix"
...
sudo cp $RESTORE_PATH/var/lib/sops-nix/key.txt /var/lib/sops-nix/key.txt
sudo chown root:root /var/lib/sops-nix/key.txt && sudo chmod 600 /var/lib/sops-nix/key.txt
```
...
```fish
sudo mkdir -p /var/lib/sops-nix
set PUBLIC_KEY (sudo age-keygen -o /var/lib/sops-nix/key.txt 2>&1 | sed 's/Public key: //')
sudo chown root:root /var/lib/sops-nix/key.txt; and sudo chmod 600 /var/lib/sops-nix/key.txt
echo "keys:
  - &personal $PUBLIC_KEY
creation_rules:
...
```bash
sudo mkdir -p /var/lib/sops-nix
export PUBLIC_KEY=$(sudo age-keygen -o /var/lib/sops-nix/key.txt 2>&1 | sed 's/Public key: //')
sudo chown root:root /var/lib/sops-nix/key.txt && sudo chmod 600 /var/lib/sops-nix/key.txt
...
- `/var/lib/sops-nix/key.txt` : Decrypts sops secrets.
- `~/projects/`
- `~/notes/`
...
- The owner of a secret can read it at /run/secrets/$SECRET. Some secrets are owned by dedicated services, some by the user: 
```bash
